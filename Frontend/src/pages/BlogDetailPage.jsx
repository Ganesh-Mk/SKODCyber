...
const BlogDetailPage = () => {
  ...
  useEffect(() => {
    const fetchBlogAndAuthor = async () => {
      try {
        setLoading(true);
        const blogResponse = await axios.get(`http://localhost:3000/blog/${blogId}`);
        setBlog(blogResponse.data);

        const authorBlogsResponse = await axios.get(
          `http://localhost:3000/authorBlogs/${blogResponse.data.userId}`
        );
        setAuthorBlogs(authorBlogsResponse.data.filter(authorBlog => 
          authorBlog._id !== blogId
        ));
      } catch (err) {
    ...
    <div className="min-h-screen mt-16 bg-[#0a0a0a] text-white">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-2xl font-bold mb-2">{blog.userName || 'Anonymous'}</h2>
        ...
        {authorBlogs.length > 0 && (
          <div>
            <h2 className="text-2xl font-bold mb-6">More from {blog.userName}</h2>
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex justify-between items-center text-sm text-gray-400">
                      ...
                      <Button
                        ...
                        onClick={() => window.location.href = `/blog/${authorBlog._id}`}
                      >
                        Read more
                      </Button>
                    </div>
